...
setInterval(function(){
  var minxy = world.getHeightAt(minx);
  var maxxy = world.getHeightAt(maxx);
  ctx.fillRect(canvas.width/2 + minx, canvas.height - minxy, 1, minxy);
  ctx.fillRect(canvas.width/2 + maxx, canvas.height - maxxy, 1, maxxy);
  if(Math.random() > .995){
    new Plant().setLength(30).setIterations(3).generate(canvas.width/2 + maxx,canvas.height - maxxy, ctx);
  }
  if(Math.random() > .993){
    new Plant().setLength(30).setIterations(3).generate(canvas.width/2 + minx,canvas.height - minxy, ctx);
  }
  minx--;
  maxx++;
  if(maxx > canvas.width/2){
